...
import testGluK2.probe as probe
...
def load_sort_save(raw_file, probe_file, force=True):
    ...
    if (base_folder / "preprocessed").is_dir() and not force:
        ...
        raw_rec = raw_rec.frame_slice(start_frame=0 * fs, end_frame=300 * fs)
        recording = raw_rec.save(folder=base_folder / "preprocessed", overwrite=True, **job_kwargs)
    # sort using Kilosort3 and save
    sorting_KS3 = si.run_sorter_by_property('kilosort3', recording, grouping_property='group',
                                            working_folder=base_folder / 'results_KS3',
                                            verbose=True)
    sorting_KS3.save(folder=base_folder / "agg_KS3")
    # extract waveforms and export to Phy
    wav_ex = si.extract_waveforms(recording, sorting_KS3, folder=base_folder / "waveforms_sparse",
                                  sparse=True, overwrite=True, **job_kwargs)
    compute_spike_amplitudes(waveform_extractor=wav_ex)
    si.export_to_phy(wav_ex, output_folder=base_folder / 'phy_KS3', remove_if_exists=True,
                     compute_amplitudes=True, compute_pc_features=True, copy_binary=False,
                     **job_kwargs)
    return sorting_KS3, wav_ex
...
if __name__ == '__main__':
    ...
    raw_f = Path(r'C:\Cajal_2024April\test_data\CM_200902_151604_Anael\amplifier.dat')
    base_folder = raw_f.parent
    probe_f = Path(r'C:\Cajal_2024April\probe_files\SI_4shank_linear.json')
    kilo_path = Path('C:\Cajal_2024April\Kilosort3_modified')
    si.Kilosort3Sorter.set_kilosort3_path(kilo_path)
    sorting_KS3, we = load_sort_save(raw_f, probe_f)
